...
IF COL_LENGTH('BMSApp.tbl_Temp_Auf_Position', 'tap_delivery_date') IS NULL
BEGIN
  ALTER TABLE [BMSApp].[tbl_Temp_Auf_Position]
  ADD [tap_delivery_date] DATETIME2(7) NULL;
END;

IF COL_LENGTH('BMSApp.tbl_Temp_Auftrag', 'ta_delivery_date') IS NOT NULL
BEGIN
  UPDATE p
  SET [tap_delivery_date] = COALESCE(p.[tap_delivery_date], o.[ta_delivery_date])
  FROM [BMSApp].[tbl_Temp_Auf_Position] p
  INNER JOIN [BMSApp].[tbl_Temp_Auftrag] o
...
WHERE dc.[parent_object_id] = OBJECT_ID('BMSApp.tbl_Temp_Auftrag')
  AND c.[name] = 'ta_delivery_date';

IF @defaultConstraintName IS NOT NULL
BEGIN
  EXEC(N'ALTER TABLE [BMSApp].[tbl_Temp_Auftrag] DROP CONSTRAINT [' + REPLACE(@defaultConstraintName, ']', ']]') + N']');
END;
