...
function formatOrderMailBody({
  ...
  const lines = [
    ...
    '',
    'AUFTRAG',
    line('Kunden-Auftragsnummer', order?.customerOrderNumber),
    ...
    line('Lieferadress-ID', order?.deliveryAddressId),
  ...
  list.forEach((position, index) => {
    lines.push(
      ...
      line('VK', `${formatNumber(position?.price, 2)} EUR/t`),
      line('EP', `${formatNumber(position?.costPrice, 2)} EUR/t`),
