...
export default function TempPurchaseOrderForm() {
  ...
  React.useEffect(() => {
    let alive = true;
    Promise.all([
      apiRequest(`/temp-purchase-orders/${encodeURIComponent(id)}`),
      apiRequest('/purchase-order-loading-locations'),
    ]).then(([orderResponse, locationResponse]) => {
      if (!alive) return;
      setOrder(orderResponse?.data || null);
      setLocations(Array.isArray(locationResponse?.data) ? locationResponse.data : []);
      setLoadingCustomerId(String(locationResponse?.meta?.ownCustomerId || '').trim());
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', width: '100%', display: 'grid', gap: 1.25 }}>
      ...
      <Card><CardContent sx={{ display: 'grid', gap: 1 }}>
        ...
        <TextField label="Ladeort" select size="small" value={order.loadingLocationId ?? ''} onChange={(e) => { const option = locations.find((item) => String(item.id) === String(e.target.value)); patchOrder({ loadingLocationId: e.target.value, loadingLocationText: option?.text || '' }); }}>
      ...
      <Button variant="contained" onClick={save} disabled={saving}>{saving ? <CircularProgress size={20} /> : 'Speichern'}</Button>
    </Box>
  );
}
